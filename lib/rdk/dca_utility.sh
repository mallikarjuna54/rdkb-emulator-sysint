...
getEstbMac()
{
    ...
    estbMac=`dmcli simu getv Device.DeviceInfo.X_COMCAST-COM_WAN_MAC  | grep type: | awk '{print $5}'|tr '[:lower:]' '[:upper:]'`
    ...
    if [ "$estbMac" ]; then
       ...
       if [ "$BOX_TYPE" == "XB3" ]; then
          ...
          estbMac=`ifconfig eth0 | grep 'Link encap:' | cut -d ' ' -f7`
          echo "$estbMac"
       fi
    fi
...
getErouterIpv4()
{
    erouter_ipv4=`dmcli simu getv Device.DeviceInfo.X_COMCAST-COM_WAN_IP | grep value | awk '{print $5}'`
...
getErouterIpv6()
{
    erouter_ipv6=`dmcli simu getv Device.DeviceInfo.X_COMCAST-COM_WAN_IPv6 | grep value | awk '{print $5}'`
...
scheduleCron()
{
	...
	DiagnosticMode=`dmcli simu getv Device.SelfHeal.X_RDKCENTRAL-COM_DiagnosticMode | grep value | cut -f3 -d : | cut -f2 -d" "`
	if [ "$DiagnosticMode" == "true" ];then
	LogUploadFrequency=`dmcli simu getv Device.SelfHeal.X_RDKCENTRAL-COM_DiagMode_LogUploadFrequency | grep value | cut -f3 -d : | cut -f2 -d" "`
